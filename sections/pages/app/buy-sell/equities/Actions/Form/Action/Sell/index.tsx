...
export const Sell = ({ classButton, buttonText }: SellProps) => {
  ...
  const valueIsMajor = useMemo(
    () => stockWalletAmount < cryptoAmountValue,
    [stockWalletAmount, cryptoAmountValue]
  );
  ...
  useEffect(() => {
    ...
    newPercentage = (cryptoAmountValue / stockWalletAmount) * 100;
  ...
  }, [cryptoAmountValue, stockWalletAmount]);
  ...
  return (
    <div className={cn(styles.box, { [styles.wrong]: transactionFail })}>
      ...
      <span className={styles.wallet}>
        <Icon name="wallet" /> {stringToValue(stockWalletAmount.toString(), 12)}{' '}
        {stockSlug}
      </span>
      <Range
        values={[percentage]}
        ...
        onChange={([newValue]) => {
          ...
          const newCryptoAmount = (stockWalletAmount / 100) * newValue;
          setCryptoAmount(newCryptoAmount.toString());

          setLastChange('CRYPTO');
        }}
        disabled={stockWalletAmount === 0}
